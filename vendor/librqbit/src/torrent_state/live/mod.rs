            ...
            let full_piece_download_time = {
                ...
                match chunk_marking_result {
                    ...
                    Some(ChunkMarkingResult::PreviouslyCompleted) => {
                        ...
                        // This peer owns the piece (checked above), yet every chunk
                        // of it was already here, so the reservation is stale.
                        // Left in `inflight`, nothing would ever complete it and
                        // it would only be stolen back and forth.
                        g.get_pieces_mut()?.take_inflight(chunk_info.piece_index);
                        return Ok(());
                    }
