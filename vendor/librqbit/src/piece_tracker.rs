...
impl PieceTracker {
        ...
        S: Fn(ValidPieceIndex) -> bool,
    {
        ...
        //
        // A piece whose chunks have all arrived leaves `inflight` before its hash
        // check and joins `have` after it. Reserving it in between re-requests a
        // piece that is already on disk: every chunk of it comes back as
        // `PreviouslyCompleted` and is thrown away, nothing ever takes it out of
        // `inflight` again, and it is stolen back and forth every 10x a peer's
        // average piece time for as long as a stream keeps it in its window.
        for piece in &mut req.priority_pieces {
            if !self.chunks.is_piece_have(piece)
                && !self.inflight.contains_key(&piece)
                && !self.chunks.is_piece_awaiting_check(piece)
...
mod tests {
    ...
    #[test]
    fn test_priority_piece_awaiting_its_hash_check_is_not_reserved_again() {
        ...
        let acquire = |tracker: &mut PieceTracker, who: u8| {
            tracker.acquire_piece(AcquireRequest {
                peer: peer(who),
                peer_avg_time: None,
                priority_pieces: std::iter::once(first),
        ...
        assert!(matches!(acquire(&mut tracker, 1), AcquireResult::Reserved(p) if p == first));
        // Its only chunk arrives, and it leaves `inflight` for the hash check,
        // exactly as `on_received_piece` does.
        let data = vec![0u8; 16384];
        assert!(matches!(
            tracker.mark_chunk_downloaded(&Piece::from_data(0, 0, &data)),
            Some(ChunkMarkingResult::Completed)
        ));
        tracker.take_inflight(first);

        match acquire(&mut tracker, 2) {
            AcquireResult::Reserved(p) => assert_ne!(p, first, "reserved a piece already on disk"),
            other => panic!("expected another piece, got {other:?}"),
        }
    }
