...
impl ChunkTracker {
    ...
    /// Every chunk of the piece has arrived but its hash check has not finished.
    ///
    /// Between the last chunk and the check a piece is neither in-flight nor
    /// had, and must not be handed out again: see `PieceTracker::acquire_piece`.
    pub(crate) fn is_piece_awaiting_check(&self, id: ValidPieceIndex) -> bool {
